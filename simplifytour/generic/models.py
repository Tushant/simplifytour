...
from simplifytour.generic.managers import KeywordManager, CommentManager
from simplifytour.core.models import Slugged, Orderable
...
class ThreadedComment(Comment):
    ...
    so we can't inherit from ``SiteRelated`` in ``simplifytour.core``, and
    therefore need to set the site on ``save``. ``CommentManager``
    inherits from simplifytour's ``CurrentSiteManager``, so everything else
    ...
    def avatar_link(self):
        from simplifytour.core.templatetags.simplifytour_tags import gravatar_url
        return format_html(
