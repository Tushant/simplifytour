...
from simplifytour.utils.deprecation import get_middleware_setting
from simplifytour.utils.timezone import get_best_local_timezone
...
def set_dynamic_settings(s):
    ...
    simplifytour's settings module.
    """
    ...
    # simplifytour's settings available to its offline context,
    ...
    if "compressor" in s["INSTALLED_APPS"]:
        ...
        def simplifytour_settings():
            from simplifytour.conf import settings
            return settings
        s["COMPRESS_OFFLINE_CONTEXT"]["settings"] = simplifytour_settings

    # Ensure the simplifytour auth backend is enabled if
    # simplifytour.accounts is being used.
    if "simplifytour.accounts" in s["INSTALLED_APPS"]:
        auth_backend = "simplifytour.core.auth_backends.simplifytourBackend"
        s.setdefault("AUTHENTICATION_BACKENDS", [])
        prepend("AUTHENTICATION_BACKENDS", auth_backend)

    # Ensure Grappelli is after simplifytour in app order so that
    ...
    if "simplifytour.blog" in s["INSTALLED_APPS"]:
        append("INSTALLED_APPS", "simplifytour.generic")
    if "simplifytour.generic" in s["INSTALLED_APPS"]:
        s.setdefault("COMMENTS_APP", "simplifytour.generic")
        append("INSTALLED_APPS", "django_comments")

    # Ensure simplifytour.boot is first.
    try:
        move("INSTALLED_APPS", "simplifytour.boot", 0)
    except ValueError:
        pass
...
def real_project_name(project_name):
    """
    Used to let simplifytour run from its project template directory, in which
