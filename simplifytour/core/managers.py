...
class CurrentSiteManager(DjangoCSM):
    ...
    (accessible via threadlocals in ``simplifytour.core.request``), the
    environment variable ``simplifytour_SITE_ID`` (which can be used by
    ...
    def get_queryset(self):
        ...
        lookup = {self.__field_name + "__id__exact": Site.objects.get_current().id}
...
class DisplayableManager(CurrentSiteManager, PublishedManager,
    ...
    def url_map(self, for_user=None, **kwargs):
        ...
        Used in ``simplifytour.core.sitemaps``.
        """
        class Home:
            title = _("Home")
        home = Home()
